...
import com.github.hexsmith.blog.mapper.ContentVoMapper;
...
import com.github.hexsmith.blog.model.vo.*;
...
import com.github.hexsmith.blog.util.ZipUtils;
import com.github.hexsmith.blog.util.backup.Backup;
    ...
    public BackResponseBo backup(String bk_type, String bk_path, String fmt) throws Exception {
        ...
        if (bk_type.equals("attach")) {
            if (StringUtils.isBlank(bk_path)) {
                throw new BizException("请输入备份文件存储路径");
            }
            if (!(new File(bk_path)).isDirectory()) {
                throw new BizException("请输入一个存在的目录");
            }
        ...
        if (bk_type.equals("db")) {
            ...
            if (!sqlFile.exists()) {
                throw new BizException("数据库备份失败");
            }
            sqlFile.delete();
